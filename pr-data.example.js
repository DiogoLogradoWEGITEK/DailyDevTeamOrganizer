if (!window.PR_DATA_EXAMPLE) {
  window.PR_DATA_EXAMPLE = {
    ...
    Carol: [
      { title: "Add user authentication flow",      number: 142, repo: "my-app",      body: "Verified login, logout and token refresh on staging. All acceptance criteria pass, tested on Chrome and mobile Safari." },
      { title: "Refactor database connection pool", number: 97,  repo: "backend-api", body: "Load tested with 50 concurrent connections on staging. No dropped connections or regressions found." },
      { title: "Redesign dashboard layout",         number: 211, repo: "frontend",    body: "" }
    ]
  };
}
